...
import { Plus, Search, ArrowUpCircle, User, Building2, Calendar, Hash, Tag, X, Scale, Package, ChevronDown, ArrowUp, Layers } from '@lucide/vue'
...
const updateFilters = debounce(() => {
    router.get(route('item.stock-out.index'), { 
        search: search.value,
...
// Dynamic Item Logic: Only show items of selected type AND have stock > 0 in selected branch
const filteredItems = computed(() => {
    ...
    return props.items.filter(item => 
        ...
        availableItemIds.includes(item.itemid)
    )
})
...
const submit = () => {
    ...
    router.post(route('item.stock-out.store'), form.value, {
        preserveScroll: true,
...
<template>
    <Head :title="ui.stock_out || 'Stock Out'" />
    ...
    <div class="max-w-7xl mx-auto space-y-6 lg:space-y-8">
        ...
        <div class="flex flex-col sm:flex-row justify-between gap-4 items-start sm:items-center px-2 lg:px-0">
            <div>
                <h1 class="text-2xl lg:text-3xl font-black uppercase tracking-tight text-primary">{{ ui.stock_out || 'Stock Out' }}</h1>
                <p class="text-muted-foreground text-[10px] lg:text-sm font-medium uppercase tracking-widest mt-1 opacity-60">{{ ui.manage_stock_out || 'View and manage outgoing stock.' }}</p>
            </div>
            <button @click="openModal" class="px-6 py-3 bg-destructive text-destructive-foreground font-black uppercase tracking-widest text-xs rounded-2xl hover:bg-destructive/90 transition-all flex items-center gap-2 shadow-lg shadow-destructive/20 active:scale-95 h-12">
                <Plus class="w-4 h-4 stroke-[3]" />
                {{ ui.output_stock || 'Output Stock' }}
            </button>
        </div>
                    ...
                    <tbody class="divide-y divide-secondary/50">
                        <tr v-for="log in logs.data" :key="log.inventory_log_id" class="hover:bg-primary/5 transition-colors group">
                            ...
                            <td class="px-8 py-5 text-center">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-red-500/10 text-red-600 border border-red-500/10">
                            ...
                            <td colspan="5" class="px-8 py-20 text-center text-muted-foreground">
                                <div class="flex flex-col items-center justify-center gap-4">
                                    <div class="w-16 h-16 rounded-full bg-secondary/30 flex items-center justify-center">
                                        <ArrowUpCircle class="w-8 h-8 opacity-20" />
                                    </div>
        ...
        <div class="md:hidden space-y-4 px-2">
            <div v-for="log in logs.data" :key="log.inventory_log_id" class="bg-card rounded-3xl shadow-sm border border-secondary p-5 flex flex-col gap-4">
                <div class="flex justify-between items-start">
                    ...
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-[0.2em] bg-red-500/10 text-red-600 border border-red-500/10">
                        -{{ formatQty(log.qty) }} {{ log.unit }}
                    </span>
                </div>
                ...
                <div class="bg-card border border-secondary rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden" @click.stop>
                    <div class="px-8 py-6 border-b border-secondary flex items-center justify-between bg-secondary/10">
                        <div class="flex items-center gap-3">
                            ...
                            <h2 class="text-lg font-black uppercase tracking-tight text-primary">Output Stock</h2>
                        </div>
                    ...
                    <form @submit.prevent="submit" class="p-8 space-y-6">
                        ...
                        <div class="pt-6 flex justify-end gap-4">
                            ...
                            <button type="submit" :disabled="isSubmitting || filteredItems.length === 0" class="px-8 py-3 text-[10px] font-black uppercase tracking-widest bg-destructive text-destructive-foreground rounded-2xl hover:bg-destructive/90 transition-all disabled:opacity-50 shadow-xl shadow-destructive/20 active:scale-95">
                                {{ isSubmitting ? 'Recording...' : 'Submit Output' }}
                            </button>
                        </div>
                    </form>
                </div>
